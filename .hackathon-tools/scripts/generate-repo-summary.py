...
def main() -> int:
    ...
    parser.add_argument("--primary-path", action="append", default=[])
...
def top_level_modules(files: list[str]) -> list[str]:
    return sorted({f.split("/", 1)[0] for f in files if "/" in f and not f.startswith(".")})
...
def is_config_file(rel: str) -> bool:
    name = Path(rel).name.lower()
    return name in {"go.mod", "package.json", "pyproject.toml", ".gitignore"} or Path(rel).suffix.lower() in {".yaml", ".yml", ".toml"}
