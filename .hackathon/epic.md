...
- Building with AI: Use Claude Code / Bob to scaffold FastAPI app with pydantic models for TFC payload, implement HMAC-SHA512 verification, add async callback pattern
...
- Building with AI: Use Claude Code / Bob to create typed httpx clients for TFC APIs, implement plan JSON reduction (filter resource_changes to creates/updates/deletes/replaces), handle non-PR cases
...
- Building with AI: Use Claude Code / Bob to implement LCEL chain that takes PR body + plan summary and returns structured verdict, map verdict to TFC outcomes format
...
- Building with AI: Use Claude Code / Bob to refine prompts for nuanced cases (updates vs creates, tag-only changes, data sources, replacements), build eval harness
...
- Building with AI: Use Claude Code / Bob to containerize app, set up Cloud Run/Fly deployment, migrate secrets to platform secret store, add health checks
